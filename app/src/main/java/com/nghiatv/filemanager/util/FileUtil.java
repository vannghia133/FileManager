package com.nghiatv.filemanager.util;
...
public class FileUtil {
    public static File copyFile(File src, File des) throws Exception {
        try {
            if (src.isDirectory()) {
                if (src.getPath().equals(des.getPath())) {
                    throw new Exception();
                }

                File directory = createDirectory(des, src.getName());
    ...
    public static File createDirectory(File des, String name) throws Exception {

        File directory = new File(des, name);
        if (directory.mkdirs()) {
            return directory;
        }
    ...
    public static ArrayList<File> searchFilesName(Context context, String name) {
        ...
        if (cursor != null) {
            ...
            while (cursor.moveToNext()) {
                ...
                if (file.exists() && file.getName().startsWith(name)) {
                    list.add(file);
                }
            }
